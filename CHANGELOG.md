...
## [0.40.0] - 2026-08-17
### Fixed
- **Browser-blocked requests no longer claim the API is unreachable**: when `fetch` rejects, the API client previously surfaced "Network error — is the API reachable?" even though the page itself loaded fine. Those failures are usually the **browser** blocking the request — Chromium's Private Network Access enforcement (surfaces as `net::ERR_ACCESS_DENIED`; Firefox/LibreWolf don't enforce it), mixed-content blocking, or an extension — so the message now says the request never reached the server and points at the real fix (try incognito mode or another browser). Verified by reproduction: the same-origin upload paths work in stock Chromium 150 and in Firefox/LibreWolf; the block happens before any bytes reach nginx/FastAPI
- **PayPal checkout no longer sends a non-PayPal plan id to the gateway**: the resolved billing plan id is now validated against the gateway's id shape **before** any request is made (PayPal requires `P-...`, Stripe `price_...`). A creator enabling PayPal without pinning a plan id used to fall back to the platform default `SUBSCRIPTION_TIER_PLAN_ID` (`price_monthly_tier` — a Stripe-style placeholder), which PayPal rejected with a cryptic `INVALID_REQUEST` 400 on `/plan_id` that surfaced to the subscriber as a 500. Now `POST /subscribe` fails fast before any gateway call, and `PayPalPaymentProvider.create_subscription` raises the same actionable error before any HTTP call. Coverage: `test_gateway_subscribe.py` (factory validation + subscribe behavior) and `test_paypal_integration.py` (provider guard)
- **Subscribers never see gateway-configuration internals**: when a creator's gateway is misconfigured (e.g. PayPal without a valid plan id), the technical reason is logged server-side and `POST /subscribe` answers `502` with a generic "this creator's payment method is temporarily unavailable" — end users no longer get bootstrap instructions or plan-id shapes (previously the subscriber-facing 400/500 leaked `python -m app.payments.bootstrap_paypal` guidance)
- **PayPal plan creation is now a dashboard button**: `POST /api/creator/gateway-settings/{gateway}/plan` creates the monthly billing plan with the creator's **own stored PayPal credentials** at their tier price and saves the returned `P-...` id into their config automatically. This is the normalized flow — the old bootstrap script needed the platform **env** credentials (it failed for admins whose dashboard was configured but the container env wasn't). The settings page shows a **Create billing plan** button once the PayPal client credentials are saved; the plan id field is auto-filled and no longer marked "(optional)"
- **Plan-id shape is validated when saving gateway settings**: a PayPal plan id that doesn't start with `P-` is rejected with a clear 400 in the settings form, so a misconfigured plan id can't sneak past the green checks to fail at checkout. Docs updated (`.env.example`, `README.md`)
...
- **Anti-bot for sign-up/login**: a hidden **honeypot** ``website`` field on register/login/forgot (filled only by bots — the endpoint then silently fake-succeeds, no account, no enumeration signal) plus **client-side proof-of-work**: `GET /auth/pow-challenge` issues a stateless HMAC-signed challenge, the sign-in forms solve it in-browser with WebCrypto (SHA-256 nonce search, `frontend/src/lib/pow.ts`) and submit the proof with the request. Proofs are single-use (consumed on first success) and fresh (TTL 120 s). `AUTH_POW_DIFFICULTY=0` disables the check for dev/tests (default); `AUTH_POW_DIFFICULTY` / `AUTH_POW_TTL_SECONDS` / `AUTH_HONEYPOT_ENABLED` tune it. Coverage: `backend/tests/test_ratelimit.py` + `test_antibot.py` (27 tests)

## [0.37.0] - 2026-08-08
### Added
- **User blocking / banning**: creators can ban misbehaving subscribers from the dashboard. The **Subscribers** tab gains a **Block** button per subscriber (with a confirmation dialog explaining the consequences) plus a **Blocked** sub-view listing banned users with an **Unblock** button. Blocking is a hard, immediate revocation for that (creator, user) pair: the blocked user is demoted from ``follower`` in the shared access resolver (feed, media, stories, likes, comments and unlocks all resolve as registered), DMs to the creator are rejected (REST + WebSocket, and the chat gate reports "You have been blocked"), ``POST /subscribe`` returns 403, and any **active subscription is canceled locally** (no charge is reversed — the subscriber keeps what they paid for but loses access). Blocked users never appear in the Subscribers list (they live on the Blocked tab), the gateway ref is detached so a late webhook for an in-flight payment can never reactivate a blocked row, and unblocking restores the ability to re-subscribe
- New table ``blocked_user`` (unique per creator+user, migration `4f5e6d7c8b9a`); creator-only endpoints `GET/POST /creator/blocked` + `DELETE /creator/blocked/{user_id}` (paginated list, idempotent block with ``was_subscriber``/prior status context, self-block/creator-block rejected). Coverage: `backend/tests/test_blocks.py` (14 tests — endpoint guards, idempotency, self/creator blocks, access demotion, DM rejection, subscribe 403, subscription cancel, unblock restore, list pagination + exclusion)
- **Creator dashboard in the hamburger menu**: the shared site menu now shows a **Creator dashboard** item (→ `/admin`) for creator accounts (``user.is_creator``), with a new ``dashboard`` grid icon — so creators reach their admin panel from the home/feed/chat pages
...
- **Legal documents per creator, editable from the admin**: a new **Legal** tab (`roque-legal-settings`) in the creator dashboard lets creators edit their Terms of Service and Privacy Policy (`creator_profile.tos_text` / `privacy_text`, via the existing profile update endpoint). Blank texts fall back to **platform defaults drafted for AI-generated content** (`app/legal.py`) — they disclose that content may be AI-generated, grant only a personal non-commercial license, ban redistribution/recording and AI-model training on the content, and cover payments/no-refunds, IP, disclaimers, liability, termination and governing law (the privacy policy covers AI-content disclosure, payment processors, cookies, retention, GDPR/CCPA rights, and children's privacy) — so a creator can never leave subscribers without a policy
- **New `/legal` page** (`legal.html` + `roque-legal-page`, nginx `/legal` route): public Terms of Service / Privacy Policy page with a two-tab layout (the shared `roque-tabs`), the creator's avatar + name, and deep links — `?tab=privacy` opens the privacy document directly. The checkout consent checkboxes link here (new tab), and the landing payload (`CreatorLandingProfileOut`) now carries the effective `tos_text` / `privacy_text` so the docs are always available pre-payment
